...
-(void)moveToNext
{
     [self scrollRectToVisible:CGRectMake(self.contentOffset.x + ([UIScreen mainScreen].bounds.size.width) / 5, 0, self.frame.size.width, 1) animated:YES];
    [self customeSnap];
    //[self scrollRectToVisible:CGRectMake(self.contentOffset.x + 50, 0, self.frame.size.width, 1) animated:YES];
}
