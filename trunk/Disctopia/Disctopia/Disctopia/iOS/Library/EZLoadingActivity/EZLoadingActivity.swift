...
public struct EZLoadingActivity {
    ...
    public static func show(text: String, disableUI: Bool) -> Bool {
        guard instance == nil else {
           BaseVC.sharedInstance.DLog("EZLoadingActivity: You still have an active activity, please stop that before creating a new one")
            return false
        }
        
        guard topMostController != nil else {
           BaseVC.sharedInstance.DLog("EZLoadingActivity Error: You don't have any views set. You may be calling them in viewDidLoad. Try viewDidAppear instead.")
            return false
        }
    ...
    public static func hide(success success: Bool? = nil, animated: Bool = false) -> Bool {
        guard instance != nil else {
           BaseVC.sharedInstance.DLog("EZLoadingActivity: You don't have an activity instance")
            return false
        }
        
        guard hidingInProgress == false else {
           BaseVC.sharedInstance.DLog("EZLoadingActivity: Hiding already in progress")
            return false
        }
    ...
    private class LoadingActivity: UIView {
        ...
        convenience init(text: String, disableUI: Bool) {
            ...
            appDelegate.navigationController?.visibleViewController?.view.addSubview(self)
...
private var topMostController: UIViewController? {
    let presentedVC = appDelegate.navigationController?.topViewController
