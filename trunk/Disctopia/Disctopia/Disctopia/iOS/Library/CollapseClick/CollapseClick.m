...
-(void)repositionCollapseClickCellsBelowIndex:(NSInteger)index withOffset:(float)offset {
    for (NSInteger yy = index+1; yy < self.dataArray.count; yy++) {
...
-(void)didSelectCollapseClickButton:(UIButton *)titleButton {
    ...
    // Cell is OPEN -> CLOSED
    if ([[self.isClickedArray objectAtIndex:(NSInteger)titleButton.tag] boolValue] == YES) {
        [self closeCollapseClickCellAtIndex:(NSInteger)titleButton.tag animated:YES];
    }
    // Cell is CLOSED -> OPEN
    else {
        [self openCollapseClickCellAtIndex:(NSInteger)titleButton.tag animated:YES];
        isOpen = YES;
    }
    
    // Call delegate method
    if ([(id)CollapseClickDelegate respondsToSelector:@selector(didClickCollapseClickCellAtIndex:isNowOpen:)]) {
        [CollapseClickDelegate didClickCollapseClickCellAtIndex:(NSInteger)titleButton.tag isNowOpen:isOpen];
    }
}
