...
@IBDesignable public class HoshiTextField: TextFieldEffects {
    ...
    */
    @IBInspectable dynamic public var placeholderFontScale: CGFloat = 0.70 {
        didSet {
            updatePlaceholder()
        }
    }
...
//        activeBorderLayer.backgroundColor = borderActiveColor?.CGColor
    ...
    private func layoutPlaceholderInTextRect() {        
        ...
        placeholderLabel.frame = CGRect(x: originX, y: placeholderInsets.y /*textRect.height/2*/,
            width: placeholderLabel.bounds.width, height: placeholderLabel.bounds.height)
        activePlaceholderPoint = CGPoint(x: placeholderLabel.frame.origin.x , y: placeholderLabel.frame.origin.y - (placeholderLabel.frame.origin.y * 1.7) )//- placeholderLabel.frame.size.height - placeholderInsets.y)
    ...
    override public func editingRectForBounds(bounds: CGRect) -> CGRect {
        return CGRectOffset(bounds, textFieldInsets.x + 10, textFieldInsets.y)
    }
