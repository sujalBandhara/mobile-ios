...
public class MenuItemView: UIView {
    ...
    public internal(set) var selected: Bool = false {
        didSet {
            ...
            case .LibraryText(let title):
                updateLibraryLabel(titleLabel, text: title)
    ...
    internal init(menuOptions: MenuViewCustomizable, menuItemOptions: MenuItemViewCustomizable, addDiveder: Bool) {
        ...
        case .LibraryText(let title):
            commonInit({
                self.setupLibraryTitleLabel(title)
                self.layoutLabel()
            })
        case .MultilineText(let title, let description):
            commonInit({
    ...
    private func setupLibraryTitleLabel(text: LibraryMenuItemText) {
        setupLibraryLabel(titleLabel, text: text)
    }
    ...
    private func setupLibraryLabel(label: UILabel, text: LibraryMenuItemText) {
        label.text = text.text
        updateLibraryLabel(label, text: text)
        addSubview(label)
    }
    private func updateLibraryLabel(label: UILabel, text: LibraryMenuItemText) {
        label.textColor = selected ? text.selectedColor : text.color
        label.font = selected ? text.selectedFont : text.font
    }
