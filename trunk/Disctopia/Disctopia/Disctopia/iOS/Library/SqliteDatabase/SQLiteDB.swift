...
let  DATABASE_NAME  =   "appDataBase.sqlite"
...
class SQLiteDB {
	...
	required init(gid:String) {
		...
		if !(fm.fileExistsAtPath(path)) {
			...
			if let from = NSBundle.mainBundle().resourcePath?.stringByAppendingPathComponent(dbName) {
				...
				do {
					...
					error = error1
					DLog("SQLiteDB - failed to copy writable version of DB!")
		...
		if error != SQLITE_OK {
			...
			DLog("SQLiteDB - failed to open DB!")
			sqlite3_close(db)
		}
		fmt.dateFormat = "YYYY-MM-dd HH:mm:ss"
	}
	...
	private func closeDatabase() {
		if db != nil {
			...
			launchCount -= 1
			DLog("SQLiteDB - Launch count \(launchCount)")
			var clean = false
			if launchCount < 0 {
				clean = true
				launchCount = 500
			...
			// Clean DB
			DLog("SQLiteDB - Optimize DB")
			let sql = "VACUUM; ANALYZE"
			if execute(sql) != SQLITE_OK {
				DLog("SQLiteDB - Error cleaning DB")
			}
			sqlite3_close(db)
		}
	}
